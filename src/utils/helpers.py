...
def get_translations(language):
    """Get translations for the specified language from JSON files"""
    ...
    translation_file = os.path.join(project_root, 'assets', 'local', f'{language}.json')
    
    # Load translations from JSON file
    translations = load_json_file(translation_file, {})
    
    # If the requested language file doesn't exist or is empty, fallback to English
    if not translations and language != 'en':
        en_translation_file = os.path.join(project_root, 'assets', 'local', 'en.json')
        translations = load_json_file(en_translation_file, {})
